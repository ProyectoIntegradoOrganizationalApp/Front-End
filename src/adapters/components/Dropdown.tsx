...
import { DropdownElement } from "../../domain/DropdownElement.interface";
...
export function Dropdown(props: { selectedElement: string, selectElement: Function, elements: Array<DropdownElement> }) {
    return (
        <>
                ...
                <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-slate-600 w-52" id="dropdown">
                    {
                        props.elements.map((element, index) =>
                            <li key={index} onClick={(event: React.MouseEvent<HTMLElement>) => { closeDropdown(); props.selectElement(element.name)}}>
